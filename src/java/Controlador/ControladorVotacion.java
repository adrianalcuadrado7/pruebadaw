...
public class ControladorVotacion extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            int id_partido= Integer.parseInt(request.getParameter("votacion"));
            HttpSession session= request.getSession(true);
            Votante v= (Votante) session.getAttribute("Votante");
            
            try{
                new Operaciones().votar(Conexion, v, id_partido);
                response.sendRedirect("Vistas/VistaMensajes.jsp?mensaje=Votacion realizada&donde=1");
